...
export default class DevRoute extends React.Component {
    ...
    renderCustomBuilds() {
        const weapons = Object.keys(DataUtility.data().weapons).map(weapon => <option key={weapon} value={weapon}>{weapon}</option>);
        const armours = Object.keys(DataUtility.data().armours).map(armour => ({name: armour, type: DataUtility.data().armours[armour].type}));
        ...
        const lanterns = Object.keys(DataUtility.data().lanterns).map(lantern => <option key={lantern} value={lantern}>{lantern}</option>);

        const cells = [];

        for(let cellName in DataUtility.data().cells) {
            let cell = DataUtility.data().cells[cellName];
    ...
    renderIcons() {
        let weapons = Object.keys(DataUtility.data().weapons).filter(weaponName =>
            !("icon" in DataUtility.data().weapons[weaponName])).sort((a, b) =>
            a.localeCompare(b)).map(weaponName =>
            <li key={weaponName}><a>{weaponName}</a></li>);

        let armours = Object.keys(DataUtility.data().armours).filter(armourName =>
            !("icon" in DataUtility.data().armours[armourName])).sort((a, b) =>
    ...
    renderTableWeapons() {
        ...
        ], Object.keys(DataUtility.data().weapons).sort().map(weaponName =>
            DataUtility.data().weapons[weaponName]
        ));
    }
    ...
    renderTableArmours() {
        ...
        ], Object.keys(DataUtility.data().armours).sort().map(name =>
            DataUtility.data().armours[name]
        ));
    }
